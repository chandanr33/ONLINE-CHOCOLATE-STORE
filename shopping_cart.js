...
// Do not use the following two methods;  they are private to this class
    ...
    this.writeCookie = function() {  // PRIVATE METHOD
        ...
        for(i=0; i < this.skuArray.length; i++) 
            toWrite += this.skuArray[i] + "|" + this.priceArray[i] + "|" + this.qtyArray[i] + "||";
...
//////////////////////////////////////////////////////////////////////////            
    ...
    this.add = function(sku, price, quantity) {
        sku = $.trim(sku);
        price = $.trim(price);
        ...
        if(this.skuArray[i] == sku) {      
            this.priceArray[i] = parseFloat(price,10) + parseFloat(this.priceArray[i],10);  
    ...
    this.setQuantity = function(sku, price, quantity) {  
        sku = $.trim(sku);
        var found = false;
        if(sku == "") return;     
        price= $.trim(price);   
